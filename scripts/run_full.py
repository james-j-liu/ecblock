...
def main():
    ...
    ap.add_argument("--include-non-gc", action="store_true",
    ...
    # Restrict the scoring pool to actual Governing Council members (Executive
    # Board + euro-area NCB governors) plus the "ECB council" composite. The raw
    # corpus also contains NCB deputies and non-euro officials who are not GC
    # members; scoring them would distort the TrueSkill ratings and the era means.
    if not args.include_non_gc:
        before = len(pool)
        pool = [s for s in pool if is_gc(s.speaker)]
        print(f"GC filter: kept {len(pool)} / {before} records "
              f"({before - len(pool)} non-GC records removed)")
